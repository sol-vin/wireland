...
describe Wireland::Circuit do
  ...
  it "should be able to connect components" do
    ...
    if start = circuit.components.find { |c| c.is_a? Wireland::Component::Start && c.size == 4 }
      start.connects.all? do |w_c|
        (circuit[w_c].is_a? Wireland::Component::Buffer && circuit[w_c].size == 4)
      end.should be_true
    else
      # Fail
    ...
    if wire = circuit.components.find { |c| c.is_a? Wireland::Component::Wire && c.size == 4 }
      wire.connects.all? do |w_c|
        (circuit[w_c].is_a? Wireland::Component::Buffer && circuit[w_c].size == 4) ||
          (circuit[w_c].is_a? Wireland::Component::Join && circuit[w_c].size == 8)
      end.should be_true
    else
      # Fail
      true.should be_false
    ...
    if altwire = circuit.components.find { |c| c.is_a? Wireland::Component::AltWire && c.size == 8 }
    ...
    if join = circuit.components.find { |c| c.is_a? Wireland::Component::Join && c.size == 8 }
      join.connects.all? do |w_c|
        (circuit[w_c].is_a? Wireland::Component::Wire && circuit[w_c].size == 4) ||
          (circuit[w_c].is_a? Wireland::Component::AltWire && circuit[w_c].size == 8) ||
          (circuit[w_c].is_a? Wireland::Component::Cross && circuit[w_c].size == 1) ||
          (circuit[w_c].is_a? Wireland::Component::NotIn && circuit[w_c].size == 4)
      end.should be_true
    ...
    circuit.components.select(&.is_a? Wireland::Component::Cross).size.should eq 4

    if not_in = circuit.components.find { |c| c.is_a? Wireland::Component::NotIn && c.size == 4 }
      not_in.connects.all? do |w_c|
        (circuit[w_c].is_a? Wireland::Component::NotOut && circuit[w_c].size == 4)
      end.should be_true
    ...
    if switch = circuit.components.find { |c| c.is_a? Wireland::Component::Switch && c.size == 4 }.as Wireland::Component::Switch
    ...
    circuit.components.any? { |c| c.is_a? Wireland::Component::NOPole && c.size == 4 && c.terminal? }.should be_true

    if no_pole = circuit.components.find { |c| c.is_a? Wireland::Component::NOPole && c.size == 4 && c.connects.size != 0 }
      no_pole.connects.all? do |w_c|
        (circuit[w_c].is_a? Wireland::Component::DiodeIn && circuit[w_c].size == 4) ||
          (circuit[w_c].is_a? Wireland::Component::NCPole && circuit[w_c].size == 4)
      end.should be_true
    ...
    if diode_in = circuit.components.find { |c| c.is_a? Wireland::Component::DiodeIn && c.size == 4 }
      diode_in.connects.all? do |w_c|
        (circuit[w_c].is_a? Wireland::Component::DiodeOut && circuit[w_c].size == 4)
      end.should be_true
    ...
    if diode_out = circuit.components.find { |c| c.is_a? Wireland::Component::DiodeOut && c.size == 4 }
    ...
    # Find middle tunnel
    tunnel = circuit.components.find!(&.is_a?(Wireland::Component::Tunnel))
  ...
  it "should be able to run pulse-test" do
    ...
    start = circuit.components.find!(&.is_a? Wireland::Component::Start)
    buffer = circuit.components.find!(&.is_a? Wireland::Component::Buffer).as(Wireland::Component::Buffer)
